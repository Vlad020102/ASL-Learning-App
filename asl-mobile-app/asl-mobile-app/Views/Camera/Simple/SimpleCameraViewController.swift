...
class SimpleCameraViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate, HandLandmarkerServiceLiveStreamDelegate, SignTargetDelegate {
    ...
    func processHandLandmarks(_ result: HandLandmarkerResult?, in viewController: UIViewController) {
        ...
        if let handLandmarks = result?.landmarks {
            DispatchQueue.main.async {
                ...
                if let vc = viewController as? SimpleCameraViewController {
                                self.clearHandLandmarks()
                            }
