...
class PoseLandmarkerService: NSObject {
   ...
   **/
  func detect(image: UIImage) -> PoseResultBundle? {
    ...
    do {
      ...
      return PoseResultBundle(inferenceTime: inferenceTime, poseLandmarkerResults: [result])
    } catch {
        print(error)
        return nil
    }
  ...
  func detect(
    ...
    inferenceIntervalInMilliseconds: Double) async -> PoseResultBundle? {
    let startDate = Date()
