...
//  asl-mobile-app
//
//  Created by Rares Achim on 19.04.2025.
//
...
class FaceLandmarkerService: NSObject {
   ...
   **/
  func detect(image: UIImage) -> FaceResultBundle? {
    ...
    do {
      ...
      return FaceResultBundle(inferenceTime: inferenceTime, faceLandmarkerResults: [result])
    } catch {
        print(error)
        return nil
    }
  ...
  func detect(
    ...
    inferenceIntervalInMilliseconds: Double) async -> FaceResultBundle? {
    let startDate = Date()
